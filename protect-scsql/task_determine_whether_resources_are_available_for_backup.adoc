...
+
** 데이터베이스가 비 NetApp 스토리지에 있는 경우 `Not available for backup` * 전체 상태 * 열에 이 표시됩니다.
+
NetApp이 아닌 스토리지에 있는 데이터베이스에는 데이터 보호 작업을 수행할 수 없습니다.

** 데이터베이스가 NetApp 스토리지에 있고 보호되지 않은 경우 `Not protected` * 전체 상태 * 열에 이 표시됩니다.
** 데이터베이스가 NetApp 스토리지 시스템에 있고 보호되어 있는 경우 사용자 인터페이스는 * 전체 상태 * 열에 메시지를 표시합니다 `Backup not run` .
** 데이터베이스가 NetApp 스토리지 시스템에 있고 보호되어 있고 데이터베이스에 대해 백업이 트리거된 경우 사용자 인터페이스는 * Overall Status * 열에 메시지를 표시합니다 `Backup succeeded` .
